...
fn display_number(number: u8, enfasis: bool) -> String {
  format!(
    "{}{}{}{}",
...
fn answer() -> u32 {
  ...
  let adj_numbers: usize = 4;

  let mut greatest_product: u32 = 0;
  let mut greatest_coords = vec![(0usize, 0usize); adj_numbers];

  // Calculate the products
  for row in 0..(20 - adj_numbers) {
    for col in 0..(20 - adj_numbers) {
      // Get combinations
      let mut hor_factors = vec![0u8; adj_numbers];
      let mut ver_factors = vec![0u8; adj_numbers];
      let mut dia_factors = vec![0u8; adj_numbers];
      let mut crs_factors = vec![0u8; adj_numbers];
      for i in 0..adj_numbers {
        hor_factors[i] = number_grid[row][col + i];
        ver_factors[i] = number_grid[row + i][col];
        dia_factors[i] = number_grid[row + i][col + i];
        crs_factors[i] = number_grid[row + adj_numbers - i][col + i];
      }
      // Calculate products
      // Horizontal
      let product = hor_factors.iter().fold(1, |p, &factor| p * factor as u32);
      ...
      // Vertical
      let product = ver_factors.iter().fold(1, |p, &factor| p * factor as u32);
      if product > greatest_product {
        greatest_product = product;
        for j in 0..adj_numbers {
      ...
      // Diagonal
      let product = dia_factors.iter().fold(1, |p, &factor| p * factor as u32);
      if product > greatest_product {
        greatest_product = product;
        for j in 0..adj_numbers {
      ...
      // Cross
      let product = crs_factors.iter().fold(1, |p, &factor| p * factor as u32);
      if product > greatest_product {
        greatest_product = product;
        for j in 0..adj_numbers {
  ...
  println!(
    "The red numbers make the greatest product of {} adjacent numbers",
    adj_numbers
  );
  ...
  for row in 0..20 {
    for col in 0..20 {
      ...
      print!(
        "{} ",
        display_number(number_grid[row][col], greatest_coords.contains(&here))
      );
    }
    print!("\n");
  }
