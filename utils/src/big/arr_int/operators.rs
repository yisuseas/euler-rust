...
impl ArrInteger {
  ...
  fn times(&self, other: &ArrInteger) -> ArrInteger {
    ...
    other.digits.iter().rev().enumerate().for_each(
      |(other_idx, &other_digit)| {
        ...
        self.digits.iter().rev().for_each(|&self_digit| {
        ...
        let partial_mult: Vec<u8> =
          partial_mult.iter().rev().map(|a| *a).collect();
