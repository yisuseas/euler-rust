    ...
    pub fn times(&self, other: &StrInteger) -> StrInteger {
        ...
        other
            ...
            .for_each(|(other_idx, other_digit)| {
                ...
                partial_mult_vec.push(partial_mult.into_iter());
            });
        ...
        loop {
            let mut column_addends = Vec::new();
            for partial_mult in partial_mult_vec.iter_mut() {
                if let Some(digit) = partial_mult.next() {
                    column_addends.push(digit as u32);
                }
            }
            if column_addends.is_empty() && remainder == 0 {
                break;
            }
            remainder += column_addends.iter().sum::<u32>();
            sum_digits.push((remainder % 10) as u8);
            remainder /= 10;
        }
...
impl PartialOrd for StrInteger {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        if self == other {
            ...
            let self_digits =
                self.n_str.chars().map(|ch| ch.to_digit(10).unwrap() as u8);
            let other_digits =
                other.n_str.chars().map(|ch| ch.to_digit(10).unwrap() as u8);
